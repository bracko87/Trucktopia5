-- migrations/040_sync_truck_model_cargo_types_to_user_trucks.sql
--
-- Idempotent migration to store human-readable cargo type NAMES on user_trucks.model_load_type
-- and keep it synchronized from truck_model_cargo_types -> cargo_types via triggers.
--
...
AS $fn$
/**
 * Populate user_trucks.model_load_type (comma-separated names) on INSERT or when
 * master_truck_id changes. Uses truck_model_cargo_types -> cargo_types to derive names.
 */
DECLARE
  names text;
BEGIN
...
-- 3) Backfill existing rows in user_trucks
WITH model_names AS (
  SELECT
    tm.id AS model_id,
...
WHERE ut.master_truck_id = mn.model_id
  AND (ut.model_load_type IS DISTINCT FROM mn.names);

-- 4) Propagation function: when mapping table changes, update affected user_trucks
...
-- 5) Propagation function: when cargo_types.name changes, update affected user_trucks
