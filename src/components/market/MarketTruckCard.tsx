 ...
 * This variant has been simplified to remove the inline "Lease Truck" button
 * from the card UI so the card remains purely informational.
 *
 * NOTE: LeaseConfirmModal was intentionally removed from this card to avoid
 * presenting the lease action inline. If you want to re-add the lease action
 * elsewhere (menu, details page, etc.) we can implement that separately.
 */
 ...
 * The inline Lease button has been removed so this component only shows model info.
 *
 * IMPORTANT FIX:
 * assetModelId now ONLY resolves to truck_models.id and never falls back
 * to marketItem.id, which previously caused invalid model fetches.
 */
export default function MarketTruckCard({ marketItem, modelId }: MarketTruckCardProps) {
  ...
  useEffect(() => {
    ...
    async function loadModel() {
      setLoading(true)
      try {
        // Only valid model sources
        const id = modelId ?? marketItem?.master_truck_id ?? null
        ...
        const res = await supabaseFetch(
          `/rest/v1/truck_models?id=eq.${encodeURIComponent(String(id))}&select=*&limit=1`
        )
  ...
  /**
   * FIX: Only valid truck_models id
   */
  const assetModelId =
    model?.id ??
  ...
  return (
    <div
      ...
      role="article"
      data-asset-model-id={assetModelId}
    >
