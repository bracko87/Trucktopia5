...
export default function HiredStaffCard({
   ...
   * profileLocationId
   *
   * When a founder/profile row exists, staff_profile_stats.current_location_id is stored here.
   */
  const [profileLocationId, setProfileLocationId] = React.useState<string | null>(null)
   ...
   *
   * This loader prefers hired_staff.current_location_id for drivers and falls back
   * to staff_profile_stats.current_location_id for founder/profile rows.
   */
  React.useEffect(() => {
    let mounted = true

    async function loadLocation() {
      // Prefer profileLocationId (which now includes current_city_id when available),
      // otherwise use hired_staff.current_location_id as a fallback.
      const locationId = profileLocationId ?? member?.current_location_id
      ...
      try {
        // The cities table uses `city_name` (not `name`) — select the correct column
        const { data, error } = await supabase
          .from('cities')
          .select('id,city_name')
          .eq('id', locationId)
        ...
        if (!error && data) {
          setFetchedLocation({
            ...
            name: String((data as any).city_name ?? 'Unknown'),
          })
        } else {
          // Keep undefined when city not found or request errored
  ...
  }, [member?.current_location_id, profileLocationId])
  ...
  React.useEffect(() => {
    ...
    async function loadProfileStats() {
      ...
      try {
        ...
        if (staffProfileId) {
          // Include current_city_id so founder/profile card can use it
          res = await supabase
            .from('staff_profile_stats')
            .select('fatigue,happiness,current_location_id,current_city_id')
        ...
        if (!res.error && res.data) {
          ...
          // Save profile current_city_id for fallback location resolution
          setProfileLocationId(((res.data as any).current_city_id ?? (res.data as any).current_location_id) ?? null)
        }
      } catch {
