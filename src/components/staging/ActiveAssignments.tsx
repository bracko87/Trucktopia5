 ...
 *
 * Shows active assignments for the logged-in user's carrier company.
 * If RLS blocks reads, PostgREST returns [] (no error) — we show a helpful hint.
 */
    ...
    ;(async () => {
      ...
      try {
        ...
        const jaUrl =
          ...
          `&select=id,status,accepted_at,job_offer:job_offer_id(` +
        ...
        if (jaRows.length === 0) {
          // This is the classic symptom of missing RLS SELECT policy.
          setDebugHint(
            'No rows returned from job_assignments. If you KNOW assignments exist, this is almost certainly an RLS SELECT policy issue on job_assignments/driving_sessions.'
          )
          if (mounted) setActiveJobs([])
          return
        }

        const assignmentIds = jaRows.map((r) => r.id).filter(Boolean)
        ...
        const dsRows = dsRes.ok ? ((await dsRes.json().catch(() => [])) as any[]) : []

        const latestByAssignment: Record<string, any> = {}
        for (const s of dsRows) {
          const k = String(s.job_assignment_id ?? '')
          if (!k) continue
          if (!latestByAssignment[k]) latestByAssignment[k] = s
        }

        const mapped: JobRow[] = jaRows
          .map((row) => {
            const offer = row.job_offer ?? {}
            const ds = latestByAssignment[String(row.id)] ?? null
            const phaseOrStatus = ds?.phase ?? row.status ?? null
          ...
          .filter((j) => isActivePhaseOrStatus(j.driving_session_phase ?? j.assignment_status ?? j.status))
