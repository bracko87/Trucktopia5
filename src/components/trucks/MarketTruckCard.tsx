...
/**
 * Minimal shape for a market list row passed from the list.
 */
...
/**
 * Minimal shape for the truck_models table row used by the card.
 */
 ...
 * MarketTruckCard
 *
 * Fetches the truck model from public.truck_models and renders a read-only card.
 *
 * @param props - MarketTruckCardProps
 * @returns JSX.Element
 */
export default function MarketTruckCard({ marketItem, modelId }: MarketTruckCardProps) {
  ...
  useEffect(() => {
     ...
     * loadModel
     *
     * Fetches a single truck model row by id using supabase REST helper.
     */
    async function loadModel() {
      setLoading(true)
      try {
        const id = modelId ?? marketItem?.master_truck_id ?? marketItem?.id ?? null
        if (!id) {
          if (mounted) {
            setModel(null)
            setLoading(false)
        ...
        const res = await supabaseFetch(`/rest/v1/truck_models?id=eq.${encodeURIComponent(String(id))}&select=*&limit=1`)
  ...
  // Compose display name using nullish coalescing only
  ...
  const condition = marketItem?.condition_score ?? marketItem?.condition ?? 0
  const status = (marketItem?.status ?? 'available').toString()
  const image = model?.image_url ?? marketItem?.image_url ?? marketItem?.thumbnail ?? undefined

  return (
    <div className="modern-card relative w-full rounded-lg bg-white overflow-visible border border-gray-200" role="article">
      <div className="flex items-center gap-4 p-3 w-full">
          ...
          <div className="min-w-0 w-full">
            <div className="flex items-center gap-3 w-full">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="text-sm font-medium truncate">{loading ? 'Loading…' : displayName}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 ml-6 flex-shrink-0">
          <div className="flex items-center gap-2 min-w-[88px]">
            ...
            <div className="min-w-0">
              ...
              <div className="text-sm font-medium text-slate-800 truncate">{condition}</div>
            </div>
          </div>
        ...
        {/* Removed location block here as requested. Menu button stays aligned on the right. */}
        <div className="ml-auto flex items-center gap-4">
          <div className="relative">
            <button type="button" aria-haspopup="true" aria-expanded="false" aria-label="Open truck details" className="p-2 rounded-md hover:bg-gray-100 text-slate-600">
